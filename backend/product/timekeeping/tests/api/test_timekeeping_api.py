...
class TimekeepingApiTests(TestCase):
    ...
    def _create_period(self, name: str = "Q1 2025"):
        return timekeeping_router.create_period(
            self.tenant.id,
            PeriodIn(name=name, start_date=date(2025, 1, 1), end_date=date(2025, 3, 31)),
            self.user,
        )
    ...
    def test_list_periods_returns_all(self):
        ...
        timekeeping_router.create_period(
            self.tenant.id,
            PeriodIn(name="Q2 2025", start_date=date(2025, 4, 1), end_date=date(2025, 6, 30)),
            self.user,
        )
    ...
    def test_submit_time_report_changes_status(self):
        ...
        submitted = timekeeping_router.submit_time_report(self.tenant.id, report.id, self.user)
    ...
    def test_approve_time_report(self):
        ...
        self._add_entry(report.id)
        timekeeping_router.submit_time_report(self.tenant.id, report.id, self.user)
        approved = timekeeping_router.approve_time_report(self.tenant.id, report.id, self.user)
    ...
    def test_reject_time_report(self):
        ...
        rejected = timekeeping_router.reject_time_report(
            self.tenant.id, report.id, RejectReportRequest(reason="Needs review"), self.user
        )
    ...
    def test_list_report_history(self):
        ...
        self._add_entry(report.id)
        timekeeping_router.submit_time_report(self.tenant.id, report.id, self.user)
        history = timekeeping_router.list_report_history(self.tenant.id, report.id, self.user)
    ...
    def test_list_time_entries(self):
        ...
        entries = timekeeping_router.list_time_entries(self.tenant.id, report.id, self.user)
    ...
    def test_update_time_entry(self):
        ...
        updated = timekeeping_router.update_time_entry(
            self.tenant.id, report.id, entry.id, TimeEntryUpdate(hours=Decimal("4")), self.user
        )
    ...
    def test_delete_time_entry(self):
        ...
        report = self._create_report(period.id)
        entry = self._add_entry(report.id)
        timekeeping_router.delete_time_entry(self.tenant.id, report.id, entry.id, self.user)
        entries = timekeeping_router.list_time_entries(self.tenant.id, report.id, self.user)
